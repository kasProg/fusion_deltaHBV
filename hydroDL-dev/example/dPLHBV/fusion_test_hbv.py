...
base_forType = 'avg_prcp'
hbv_path = "/data/kas7897/dPLHBVrelease/output/CAMELSDemo/dPLHBV_capillary/ALL" \
              f"/TDTestforc/TD1_13/{base_forType}/BuffOpt0/NSE/111111/Fold1" \
...
Nfea = 14 # number of HBV parameters, 13 includes the added one for ET eq; #14 for capillary
BUFFTIME = 365
routing = True
Nmul = 16
comprout = False
...
## Testing parameters
Ttest = [19951001, 20081001]  # testing period
# Ttest = [19891001, 19991001]  # PUB/PUR period
TtestLst = utils.time.tRange2Array(Ttest)
TtestLoad = [19951001, 20081001]  # could potentially use this to load more forcings before testing period as warm-up
...
predtestALL = np.full([len(gageid), len(TtestLst),6], np.nan)
...
if forType == ['daymet', 'maurer_extended', 'nldas_extended']:
    testsave_path = 'CAMELSDemo/LSTM-dPLHBV/' + puN + '/TDTestforc/' + 'TD' + "_".join(tdRepS) + '/allprcp_35withloss'+str(prcp_loss_factor)+'smooth'+str(smooth_loss_factor)+'/BuffOpt' + str(buffOptOri) + '/RMSE_para'+ str(alpha) + '/' + str(testseed)
...
for ifold in range(1, len(tarIDLst)+1):
    ...
    if base_forType == 'daymet':
        varF_base = ['prcp', 'tmean']
        varFInv_base = ['prcp', 'tmean']
    else:
        varF_base = ['prcp', 'tmax']  # For CAMELS maurer and nldas forcings, tmax is actually tmean
        varFInv_base = ['prcp', 'tmax']

    dfTrain_base = camels.DataframeCamels(tRange=TtrainLoad, subset=TrainLS, forType=base_forType)
    forcUN_base = dfTrain_base.getDataTs(varLst=varF_base, doNorm=False, rmNan=False)
    ...
    dfTest_base = camels.DataframeCamels(tRange=TtestLoad, subset=TestLS, forType=base_forType)
    forcTestUN_base = dfTest_base.getDataTs(varLst=varF_base, doNorm=False, rmNan=False)
    ...
    series_Wghts_norm = camels.transNormbyDic(series_inv,['prcp_daymet', 'prcp_maurer', 'prcp_nldas','tmax',
                                                                                         'pet'], statDict_wghts, toNorm=True)
    ...
    seriestest_inv_hbv = np.concatenate([forcTestUN_base, PETTestUN], axis=2)
    #for multiforcing
    ...
    xTrain_hbv = np.concatenate([forcUN_base, PETUN], axis=2) # used as HBV forcing
    ...
    filePathLst = master.master.namePred(
          testout, Ttest, 'All_Buff'+str(TestBuff), epoch=testepoch, targLst=['Qr', 'Q0', 'Q1', 'Q2', 'ET', 'SWE'])
    ...
    if PUOpt == 0: # for ALL basins, temporal generalization test
        ...
        xTest_hbv = np.concatenate([forcTestUN_base, PETTestUN], axis=2)
    ...
    # zTest_hbv = np.concatenate([zTest_wghts, cTemp_hbv], 2) # Add attributes to historical forcings as the inversion part
    ...
    loaded_hbv = torch.load(hbv_path, map_location=torch.device(f"cuda:{testgpuid}"))
    loaded_hbv.inittime = runBUFF
    loaded_hbv.dydrop = 0.0
    loaded_hbv.staind = TestBuff - 1
    # forward the model and save results
    train.test2Model(
        testmodel, loaded_hbv, testTuple_wghts, testTuple_hbv, c=attrs_hbv, batchSize=testbatch,
