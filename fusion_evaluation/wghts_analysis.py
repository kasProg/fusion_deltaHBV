...
eva0 = np.load(f"/data/kas7897/dPLHBVrelease/output/CAMELSDemo/LSTM-dPLHBV/ALL/TDTestforc/TD1_13/"
               f"allprcp_{expno}withloss0smooth0/BuffOpt0/RMSE_para0.25/111111/Train19801001_19951001Test19951001_20081001Buff5478Staind5477/Eva50.npy", allow_pickle=True)
daymet_eva = np.load(
    f'/data/kas7897/dPLHBVrelease/output/CAMELSDemo/dPLHBV_capillary/ALL/TDTestforc/'
    f'TD1_13/daymet/BuffOpt0/NSE/111111/Train19801001_19951001Test19951001_20081001Buff5478Staind5477/Eva50.npy', allow_pickle=True)

lstm_daymet_eva = np.load("/data/kas7897/testing_hydrodl/hydroDL-release/output/"
                          "rnnStreamflow/CAMELSDemo_daymet111111/TestRun/"
                          "epochs300_batch100_rho365_hiddensize256_Tstart19801001_Tend19951001_trainBuff365_flowregime1/All/Eva.npy", allow_pickle=True)


lstm_fused_eva = np.load("/data/kas7897/testing_hydrodl/hydroDL-release/output/"
                          "rnnStreamflow/CAMELSDemo_27fused111111/TestRun/"
                          "epochs300_batch100_rho365_hiddensize256_Tstart19801001_Tend19951001_trainBuff365_flowregime1/All/Eva.npy", allow_pickle=True)


fusion_eva_sorted = np.sort(eva0[0]["NSE"])
dplhbv_eva_sorted = np.sort(daymet_eva[0]["NSE"])
lstm_daymet_eva_sorted = np.sort(lstm_daymet_eva[0]["NSE"])
lstm_fused_eva_sorted = np.sort(lstm_fused_eva[0]["NSE"])

low_basins_improve = lstm_fused_eva[0]['NSE'][np.where(lstm_daymet_eva[0]["NSE"] < 0.5)[0]] - lstm_daymet_eva[0]['NSE'][np.where(lstm_daymet_eva[0]["NSE"] < 0.5)[0]]

fusion_cdf = np.arange(1, len(fusion_eva_sorted) + 1) / len(fusion_eva_sorted)
dplhbv_cdf = np.arange(1, len(dplhbv_eva_sorted) + 1) / len(dplhbv_eva_sorted)
lstm_daymet_cdf = np.arange(1, len(lstm_daymet_eva_sorted) + 1) / len(lstm_daymet_eva_sorted)
lstm_fused_cdf = np.arange(1, len(lstm_fused_eva_sorted) + 1) / len(lstm_fused_eva_sorted)


# Plot the CDFs
plt.plot(fusion_eva_sorted, fusion_cdf, linestyle='-', color='red', label='dpl-Fusion')
plt.plot(dplhbv_eva_sorted, dplhbv_cdf, linestyle='--', color='blue', label='dplHBV (Daymet)')
plt.plot(lstm_daymet_eva_sorted, lstm_daymet_cdf, linestyle='--', color='green', label='LSTM (Daymet)')
plt.plot(lstm_fused_eva_sorted, lstm_fused_cdf, linestyle='-', color='black', label='LSTM (Fused P)')
...
evaDictLst = list(eva0) + evaS531Dict
...
axs[0].set_title(f'(a) NSE - HydroDL', size=12)
m.ax = axs[0]
...
scatter = m.scatter(x, y, c=lstm_daymet_eva[0]['NSE'], cmap='seismic', alpha=0.9, zorder=2, vmin = 0.0, s=20)
...
axs[1].set_title(f'(b) NSE - δHBV',size=12)
m.ax = axs[1]
m.etopo(scale=0.5, alpha=0.5)
m.drawcoastlines()
m.drawcountries(linewidth=1)
...
scatter = m.scatter(x, y, c=daymet_eva[0]['NSE'], cmap='seismic', alpha=0.9, zorder=2, vmin=0, s=20)
cax4 = fig.add_axes([0.55, 0.32, 0.35, 0.02])
cbar = plt.colorbar(scatter, cax=cax4, shrink=0.9, orientation='horizontal', pad=0.05, extend='min')
...
w0 = pd.read_csv(
    f'/data/kas7897/dPLHBVrelease/output/CAMELSDemo/LSTM-dPLHBV/ALL/TDTestforc/TD1_13/allprcp_{expno}withloss{lossfactor}smooth{smoothfactor}/BuffOpt0/RMSE_para0.25/111111/Fold1/T_19801001_19951001_BS_100_HS_256_RHO_365_NF_14_Buff_365_Mul_16/prcp_wghts1.csv',
    header=None)

w1 = pd.read_csv(
    f'/data/kas7897/dPLHBVrelease/output/CAMELSDemo/LSTM-dPLHBV/ALL/TDTestforc/TD1_13/allprcp_{expno}withloss{lossfactor}smooth{smoothfactor}/BuffOpt0/RMSE_para0.25/111111/Fold1/T_19801001_19951001_BS_100_HS_256_RHO_365_NF_14_Buff_365_Mul_16/prcp_wghts2.csv',
    header=None)

w2 = pd.read_csv(
    f'/data/kas7897/dPLHBVrelease/output/CAMELSDemo/LSTM-dPLHBV/ALL/TDTestforc/TD1_13/allprcp_{expno}withloss{lossfactor}smooth{smoothfactor}/BuffOpt0/RMSE_para0.25/111111/Fold1/T_19801001_19951001_BS_100_HS_256_RHO_365_NF_14_Buff_365_Mul_16/prcp_wghts3.csv',
    header=None)
...
# plt.tight_layout()
plt.savefig(f'/home/kas7897/final_plots_fusion_paper/wghts{expno}.png',  dpi=300, bbox_inches='tight')
...
scatter = m.scatter(x, y, c=eva0[0]['NSE'], cmap='seismic', alpha=0.9, zorder=2, vmin = 0.0, s=15)
...
scatter = m.scatter(x, y, c=delta_nse_daymet, cmap='seismic', alpha=0.9, zorder=2, vmin=-0.35, vmax=0.35,s=15)
...
scatter = m.scatter(x, y, c=eff_wght_daymet, cmap=cmap, alpha=0.9, zorder=2, s=15)
# scatter = m.scatter(x, y, c=time_avg_slope_wo[0], cmap='seismic', alpha=0.9, zorder=2)
# cbar = plt.colorbar(label='wsum (sum of weights)', shrink=0.92, orientation='horizontal', pad=0.05,  fontsize=16)
...
scatter = m.scatter(x, y, c=eff_wght_maurer, cmap=cmap, alpha=0.9, zorder=2, s=15)
...
scatter = m.scatter(x, y, c=eff_wght_nldas, cmap=cmap, alpha=0.9, zorder=2, s=15)
# scatter = m.scatter(x, y, c=time_avg_slope_w2[0], cmap='seismic', alpha=0.9, zorder=2)
# cbar = plt.colorbar(label='wsum (sum of weights)', shrink=0.92, orientation='horizontal', pad=0.05,  fontsize=16)
# cax5 = fig.add_axes([0.13, 0.26, 0.35, 0.02])
cax5 = fig.add_axes([0.125, 0.08, 0.35, 0.02])
cbar = plt.colorbar(scatter, cax=cax5, shrink=0.6, orientation='horizontal', pad=0.05)
# cbar.set_label(f'w2(nldas), loss factor {lossfactor}; smoothing factor {smoothfactor}', size=7)
# plt.title(f'(c) w\u2082 (weight associated to NLDAS2)', size=12)
cbar.ax.tick_params(labelsize=10)
...
scatter = m.scatter(x, y, c=(eff_wght_daymet+eff_wght_maurer+eff_wght_nldas), vmin = 0.92, vmax=1.08, cmap='seismic', alpha=0.9, zorder=2, s=15)
# cbar = plt.colorbar(label='wsum (sum of weights)', shrink=0.92, orientation='horizontal', pad=0.05,  fontsize=16)
...
# plt.title(f'Sum of Weights (w\u2080 + w\u2081 + w\u2082)')
cbar.ax.tick_params(labelsize=10)
plt.savefig(f'/home/kas7897/final_plots_fusion_paper/nse_eff_wght_{expno}.png', dpi=300, bbox_inches='tight')
